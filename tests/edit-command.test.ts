...
describe("buildEditPrompt", () => {
  it("includes edit target details and safety workflow", () => {
    ...
    expect(prompt).toContain("Before patching `decks/*.html`")
    expect(prompt).toContain("`revela-designs`")
    expect(prompt).toContain('`section: "rules"`')
    expect(prompt).toContain("fetch the active design rules")
    expect(prompt).toContain("fetch the relevant `revela-designs` layout/component details")
    expect(prompt).toContain('`section: "chart-rules"`')
    expect(prompt).toContain("Do not invent layout classes, component names, CSS variables, icon systems, or visual effects")
    expect(prompt).toContain("patch `decks/*.html` directly")
    expect(prompt).toContain("targeted `revela-decks` vault actions")
