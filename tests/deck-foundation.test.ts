...
describe("deck foundation helper", () => {
  it("creates a file-native deck shell without DECKS.json", () => {
    ...
    try {
      ...
      expect(html).toContain('<link rel="stylesheet" href="./_revela-design/demo-active/design.css">')
      ...
      expect(existsSync(join(root, "decks", "_revela-design", "demo-active", "design.css"))).toBe(true)
  ...
  it("exposes compact JSON through the tool", async () => {
    ...
    try {
      ...
      expect(result.includedSections).toContain("design-css:snapshot")
      expect(result.includedSections).toContain("design-assets:snapshot")
      expect(existsSync(join(root, "decks", "tool.html"))).toBe(true)
      expect(existsSync(join(root, "decks", "_revela-design", "tool-active", "design.css"))).toBe(true)
      expect(existsSync(join(root, "DECKS.json"))).toBe(false)
    } finally {
      rmSync(root, { recursive: true, force: true })
    }
