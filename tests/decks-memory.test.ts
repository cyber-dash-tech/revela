...
describe("buildInitPrompt", () => {
  ...
  it("updates existing DECKS.json conservatively", () => {
    ...
    expect(prompt).toContain("newer than the vault timestamp")
    expect(prompt).toContain("migration.available: true")
    expect(prompt).toContain("exportNarrativeVault")
    expect(prompt).toContain("continue by editing Markdown vault files")
  })
  ...
  it("does not make deck render inputs mandatory during narrative init", () => {
    ...
    expect(prompt).toContain("revela-narrative/")
    expect(prompt).toContain("compileNarrativeVault")
    expect(prompt).toContain("initNarrativeVault")
    expect(prompt).toContain("Distill ingested files into the Markdown vault")
    expect(prompt).toContain("claims/*.md")
    expect(prompt).toContain("## Relations")
    expect(prompt).toContain("[[claim:...]]")
    expect(prompt).toContain("Do not use `upsertNarrative`; it is deprecated")
...
describe("research command", () => {
  it("builds a closed-loop research and binding prompt", () => {
    ...
    expect(prompt).toContain("After Markdown edits, call `revela-decks compileNarrativeVault`")
    expect(prompt).toContain("then re-run `reviewNarrative` and `deriveResearchTargets`")
