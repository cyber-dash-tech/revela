...
describe("buildInitPrompt", () => {
  ...
  it("updates existing DECKS.json conservatively", () => {
    ...
    expect(prompt).toContain("forbiddenCompatibilityActions")
    ...
    expect(prompt).toContain("Direct Markdown patches are reserved for small repairs")
  })
  ...
  it("does not make deck render inputs mandatory during narrative init", () => {
    ...
    expect(prompt).toContain("initNarrativeVault")
    expect(prompt).toContain("Distill stable findings through structured vault actions from `authoringContract`")
    ...
    expect(prompt).toContain("Do not batch-create canonical meaning through raw Markdown")
    expect(prompt).toContain("pass `narrative.claimRelations` with `upsertVaultClaim`")
...
describe("review command", () => {
  it("builds a prompt that reviews narrative readiness", () => {
    ...
    expect(prompt).toContain("report blocker diagnostics before readiness blockers")
  ...
  it("builds a deck/artifact gate prompt separately", () => {
    ...
    expect(prompt).toContain("blocker diagnostics before artifact readiness")
    expect(prompt).toContain("file/node/code/message/suggestedAction")
  ...
  it("builds a deck handoff prompt from approved narrative to artifact gate", () => {
    ...
    expect(prompt).toContain("Markdown file/node/code/message")
    expect(prompt).toContain("vault diagnostic blockers or warnings")
