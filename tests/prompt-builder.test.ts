...
describe("buildPrompt", () => {
  ...
  it("builds narrative mode without design or HTML generation layers", () => {
    ...
    expect(prompt).toContain("revela-narrative/**/*.md")
  ...
  it("builds deck-render mode with the legacy design layer", () => {
    ...
    expect(prompt).toContain("Follow `htmlWritingBatches`")
    expect(prompt).toContain("at most 5 `<section class=\"slide\">`")
    expect(prompt).toContain("revela-deck-foundation")
    expect(prompt).toContain("revela-slides")
    expect(prompt).toContain("Never add or rewrite")
    expect(prompt).toContain("one listed batch at a time")
    expect(prompt).toContain("valid after every write")
    expect(prompt).toContain("deck-plan.md")
    expect(prompt).toContain("sourceLinks")
    expect(prompt).toContain("box.children")
