...
const codexReviewSkill = readFileSync(codexReviewSkillPath, "utf-8")
...
describe("Codex revela-helper skill", () => {
  it("reports Revela status, active design, and active domain without mutating artifacts", () => {
    ...
    expect(codexHelperSkill).toContain("Existing deck artifact: run `revela-review`")
    expect(codexHelperSkill).toContain("open the HTML deck directly in Codex Browser")
...
describe("Codex skill discoverability docs", () => {
  it("documents design and domain skills in the public Codex skill list", () => {
    ...
    expect(readme).toContain("`revela-review` for opening HTML decks directly in Codex Browser")
  })

  it("keeps the README Quick Start aligned with the current Codex workflow", () => {
    expect(readme).toContain("1. **Spec**")
    expect(readme).toContain("2. **Plan**")
    expect(readme).toContain("3. **Render Deck**")
    expect(readme).toContain("4. **Review**")
    expect(readme).toContain("5. **Export**")
    expect(readme).toContain("spec.md")
    expect(readme).toContain("deck-plan.md")
    expect(readme).toContain("Use Revela to review @decks/<file>.html in Codex Browser.")
    expect(readme).toContain("Use Revela to export @decks/<file>.html as editable PPTX.")
    expect(readme).not.toContain("Annotate Deck")
    expect(readme).not.toContain("Annotate A Deck")
    expect(readme).not.toContain("Narrative Vault")
    expect(readme).not.toContain("help me init this workspace")
    expect(readme).not.toContain("/Users/")
    expect(readme).not.toContain(".codex/plugins/cache")
  })
})
...
describe("Codex revela-design skill", () => {
  it("restores discoverable draft-first design authoring", () => {
    ...
    expect(codexDesignSkill).toContain("call `revela_switch_deck_design` with the deck file and design name")
  ...
  it("marks Codex design authoring as skill-backed in the capability matrix", () => {
    expect(codexCapabilityMatrix).toContain("| Design list/read/author/install/share/activate/switch deck |")
    ...
    expect(codexCapabilityMatrix).toContain("`revela_switch_deck_design` refreshes one deck's local active design snapshot")
  })
})
...
describe("Codex revela-research skill", () => {
  it("uses domain-guided material intake, tool-backed research, and deck-plan handoff", () => {
    ...
    expect(codexResearchSkill).toContain("If diagnostics report `sourceLinks` or template id issues")
...
describe("Codex revela-make-deck skill", () => {
  it("requires an existing deck-plan and design-aware render preflight before HTML generation", () => {
    ...
    expect(codexMakeDeckSkill).not.toContain("revela_design_read_layout")
    expect(codexMakeDeckSkill).not.toContain("revela_design_read_component")
    expect(codexMakeDeckSkill).toContain("pageTemplates")
    ...
    expect(codexMakeDeckSkill).toContain("Built-in page template tools")
...
describe("Codex Browser deck review", () => {
  it("publishes revela-review as the direct Codex Browser workflow", () => {
    expect(existsSync(codexReviewSkillPath)).toBe(true)
    expect(codexReviewSkill).toContain("name: revela-review")
    expect(codexReviewSkill).toContain("Call `revela_open_deck`")
    expect(codexReviewSkill).toContain("The default path opens the HTML deck itself in Codex Browser without running QA first")
    expect(codexReviewSkill).toContain("call `revela_review_deck_read` only when the user explicitly asks")
    expect(codexReviewSkill).toContain("Do not run QA before opening Review unless the user explicitly asks")
    expect(codexReviewSkill).not.toContain("revela_review_deck_open")
    expect(codexReviewSkill).not.toContain("/codex-review")
    expect(codexReviewSkill).not.toContain("promptBridge")
    expect(codexReviewSkill).not.toContain("refine panel")
    expect(codexCapabilityMatrix).toContain("| Deck review after make |")
    expect(codexCapabilityMatrix).toContain("`revela-review` skill opens")
    expect(codexCapabilityMatrix).toContain("`revela_open_deck`")
    expect(codexCapabilityMatrix).toContain("diagnostics use `revela_review_deck_read` only when requested")
    expect(codexCapabilityMatrix).not.toContain("no public `revela-review` Codex skill")
    expect(codexProductPlan).toContain("Direct Codex Browser deck viewing is the default post-make surface")
    expect(codexProductPlan).toContain("`revela_open_deck`")
    expect(codexProductPlan).toContain("without running QA first")
    expect(codexProductPlan).not.toContain("remain compatibility-only")
