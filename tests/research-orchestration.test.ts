...
describe("Codex revela-helper skill", () => {
  it("reports Revela status, active design, and active domain without mutating artifacts", () => {
    ...
    expect(codexHelperSkill).toContain("Existing deck artifact: start a read-only local static server")
    expect(codexHelperSkill).toContain("localhost website card/link")
...
describe("Codex revela-make-deck skill", () => {
  it("requires an existing deck-plan and design-aware render preflight before HTML generation", () => {
    ...
    expect(codexMakeDeckSkill).toContain("After the final `revela_run_deck_qa` passes with zero hard errors, do not open the deck file directly")
    expect(codexMakeDeckSkill).toContain("Start a read-only local static server from the workspace root")
