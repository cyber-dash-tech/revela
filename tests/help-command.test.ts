...
describe("help command", () => {
  ...
  it("explains disabled state and advertises enable/disable controls", async () => {
    ...
    expect(help).toContain("discover local sources, extract materials, and capture deck intent")
    expect(help).toContain("/revela research")
    expect(help).toContain("/revela plan --deck")
    expect(help).not.toContain("/revela story")
    expect(help).toContain("/revela make --deck")
    expect(help).toContain("/revela export --deck png")
