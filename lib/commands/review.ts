...
- When an evidence candidate includes \`evidenceDraft\`, report it as a proposed slide evidence record with its \`candidateId\`; it still requires explicit user/agent confirmation before binding. In a Markdown vault workspace, binding means creating or updating \`revela-narrative/evidence/*.md\` and running \`compileNarrativeVault\`; in compatibility JSON workspaces, binding can use \`revela-decks\` action \`applyEvidenceCandidates\`. Also report \`unsupportedScope\` and \`recommendedRewrite\` so partial evidence is not stretched to future-state claims.
...
- Treat \`evidenceDraft\` as a proposed record, not a mutation. Do not call \`upsertSlides\` to bind it. If the user asks to apply candidate bindings, edit \`revela-narrative/evidence/*.md\` and run \`compileNarrativeVault\` when a Markdown vault exists; otherwise call \`revela-decks\` action \`applyEvidenceCandidates\` with explicit \`candidateIds\`.
