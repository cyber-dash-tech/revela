...
Workflow:
1. Call \`revela-decks\` with action \`read\` and \`summary: true\` for the current workspace deck and any \`vaultDiagnostics\`.
2. If the read summary returned \`vaultDiagnostics.blockers\`, report blocker diagnostics before artifact readiness with file/node/code/message/suggestedAction and stop before \`revela-decks review\`, deck HTML writes, or export guidance until the vault blocker is fixed.
