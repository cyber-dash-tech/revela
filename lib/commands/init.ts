...
8. If enough information is available, preserve a compact compatibility \`narrativeBrief\` through \`revela-decks\` action \`upsertDeck\`: audience belief before/after, decisionOrAction, narrativeArc or thesis, keyClaims, objections, and risks. This compatibility projection lets the canonical narrative state normalize safely without requiring deck rendering.
9. If the workspace has explicit slide/deck information, existing HTML, or a user-requested deck task, you may also call \`upsertDeck\` and \`upsertSlides\` for explicit deck information. Do not pass or ask for a deck key; the tool uses the workspace folder name internally. Do not mark deck readiness ready during init.
