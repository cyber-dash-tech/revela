...
8. If enough information is available, preserve canonical narrative intent. When \`revela-narrative/\` exists, use targeted vault actions as applicable: \`updateVaultCoreNarrative\` for audience/decision/thesis/status, \`upsertVaultClaim\` for claims, \`upsertVaultEvidence\` for explicit evidence, \`upsertVaultObjection\` for objections, and \`upsertVaultRisk\` for risks. Use \`compileNarrativeVault\` after manual Markdown edits not covered by targeted actions. When no vault exists, use \`revela-decks\` action \`upsertNarrative\` for audience intent, decision intent, thesis, central claims, explicit evidence bindings where known, objections, and risks. This does not require deck rendering inputs.
