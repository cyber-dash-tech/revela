...
4. Register or refresh source material records by passing the scan result's \`sourceMaterial\` objects to \`revela-decks\` action \`init\`. Preserve unchanged existing records; the tool will upsert by path and fingerprint.
5. Select the files that look most relevant for understanding the narrative problem. Prioritize source decks, PDFs, Word docs, spreadsheets, CSVs, Markdown, text notes, and relevant existing generated artifacts.
...
13. Treat \`workspace.sourceMaterials\` as a reusable candidate index, not proof by itself. A source material record alone is not narrative evidence or slide evidence.
14. Report what was initialized, updated, or migrated. If \`exportNarrativeVault\` is recommended or executed, explicitly state that approvals, render targets, reviews, artifact coverage, actions, deck specs, and source material records stay in ${DECKS_STATE_FILE}.
