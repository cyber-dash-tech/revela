...
- Treat \`authoringContract\` returned by \`read(summary: true)\`, \`initNarrativeVault\`, or \`narrativeInventory\` as the Markdown authoring guide: valid node types, plain id convention, relation registry guidance, forbidden compatibility actions, and optional helper templates.
...
- Create/update content nodes first. Add graph edges afterward through \`revela-narrative/relations.md\` or a relation helper when available. Do not add new \`## Relations\` sections to node files; treat existing inline \`## Relations\` as legacy-compatible material to migrate later.
...
- New graph relations belong in \`revela-narrative/relations.md\` with \`id\`, \`from\`, \`to\`, \`type\`, and optional \`rationale\`. Do not write new inline \`## Relations\` sections in node files.
- Registry relations reference plain frontmatter node ids directly. Do not write typed targets such as \`[[claim:claim-belief-change-purpose]]\`.
...
- Preserve graph meaning by writing explicit edges in \`revela-narrative/relations.md\` after nodes exist. Do not create new inline \`## Relations\` sections.
