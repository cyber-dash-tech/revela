...
9. Automatically bind evidence only when all binding criteria are met and the diagnostic is \`bindable: true\` or the same fields are explicit in the findings. If no Markdown vault exists, call \`revela-decks initNarrativeVault\` first. Then call \`revela-decks upsertVaultEvidence\` to write \`revela-narrative/evidence/*.md\` with explicit source trace; inspect the returned \`diagnosticReport\` and report any remaining warnings instead of hiding them. \`compileNarrativeVault\` remains the explicit compile action after manual Markdown edits. Do not use \`upsertNarrative\` during research to add evidence or update narrative arrays.
...
- Do not call \`upsertNarrative\` during research. Initialize the Markdown vault with \`initNarrativeVault\` if needed; research may update gap nodes with \`updateVaultResearchGap\`, bind evidence with \`upsertVaultEvidence\`, and safely narrow claim text/scope with \`upsertVaultClaim\`. Broader narrative rewrites must be reported for Story/user confirmation.
