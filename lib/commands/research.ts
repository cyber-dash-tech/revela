...
9. Automatically bind evidence only when all binding criteria are met and the diagnostic is \`bindable: true\` or the same fields are explicit in the findings. When a Markdown vault exists, call \`revela-decks upsertVaultEvidence\` to write \`revela-narrative/evidence/*.md\` with explicit source trace; inspect the returned \`diagnosticReport\` and report any remaining warnings instead of hiding them. \`compileNarrativeVault\` remains the explicit compile action after manual Markdown edits. Otherwise use \`revela-decks applyEvidenceCandidates\` for concrete candidate ids when available. Do not use \`upsertNarrative\` during research to add evidence or update narrative arrays.
...
11. If a claim is broader than the evidence but can be safely narrowed without changing strategic meaning, update the claim boundary with \`upsertVaultClaim\` in vault workspaces; in compatibility JSON workspaces, report the narrowing instead of using \`upsertNarrative\` during research. Relation rewrites and strategic claim changes must be reported in \`Narrative changes\`, keep unsupported scope visible, and make \`/revela story\` or explicit user confirmation the next action.
...
- Do not call \`upsertNarrative\` during research. In vault workspaces, research may update gap nodes with \`updateVaultResearchGap\`, bind evidence with \`upsertVaultEvidence\`, and safely narrow claim text/scope with \`upsertVaultClaim\`; in compatibility JSON workspaces, research may update gaps, attach findings, and apply explicit evidence candidates. Broader narrative rewrites must be reported for Story/user confirmation.
