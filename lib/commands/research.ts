...
9. Automatically bind evidence only when all binding criteria are met and \`bindingEval.status\` is \`bindable\` with an explicit \`claimId\` and \`recommendedEvidenceDraft\`, or the same fields are explicit in the findings. If no Markdown vault exists, call \`revela-decks initNarrativeVault\` first. Then write \`revela-narrative/evidence/*.md\` directly with explicit frontmatter fields: \`id\`, \`claimId\`, \`source\`, \`sourcePath\` or \`url\` or \`findingsFile\`, \`quote\`, \`supportScope\`, \`unsupportedScope\`, \`caveat\`, and \`strength\`. Add \`## Relations\` in the relevant claim file when an explicit relation is needed, for example \`- depends_on: [[evidence:...]]\`. Use \`upsertVaultEvidence\` only as a deterministic helper when direct Markdown editing is unavailable or unsafe.
...
- Do not call \`upsertNarrative\` during research. Initialize the Markdown vault with \`initNarrativeVault\` if needed; research should author canonical changes by editing \`revela-narrative/evidence/*.md\`, \`research-gaps/*.md\`, and safe \`claims/*.md\` directly, then compile. Targeted vault actions are fallback helpers, not the primary writing path. Broader narrative rewrites must be reported for Story/user confirmation.
