...
2. If current research gaps are missing or stale, add or refresh them through \`revela-narrative/research-gaps/*.md\` and then call \`compileNarrativeVault\` when a Markdown vault exists; otherwise call \`deriveResearchGaps\` when useful. Then call \`deriveResearchTargets\` again. Do not invent gaps that are not tied to a claim, objection, risk, decision, or narrative issue.
...
9. Automatically bind evidence only when all binding criteria are met and the diagnostic is \`bindable: true\` or the same fields are explicit in the findings. When a Markdown vault exists, create or update \`revela-narrative/evidence/*.md\` with explicit source trace and run \`compileNarrativeVault\`; otherwise use \`revela-decks applyEvidenceCandidates\` for concrete candidate ids when available. Do not use \`upsertNarrative\` during research to add evidence or update narrative arrays.
...
- Do not call \`upsertNarrative\` during research. In vault workspaces, research may edit research-gap or evidence Markdown nodes and run \`compileNarrativeVault\`; in compatibility JSON workspaces, research may update gaps, attach findings, and apply explicit evidence candidates. Broader narrative rewrites must be reported for Story/user confirmation.
