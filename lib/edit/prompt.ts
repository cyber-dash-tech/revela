...
- Preserve the narrative boundary: if the requested edit changes audience framing, belief shift, decision/action, thesis, recommendation, claim wording, evidence scope, caveat, risk, objection, or decision ask, do not patch the HTML directly. Explain that the canonical narrative must be updated first by editing ${"`revela-narrative/**/*.md`"} and running ${"`revela-decks`"} action ${"`compileNarrativeVault`"} when a Markdown vault exists, or through ${"`revela-decks`"} action ${"`upsertNarrative`"} in compatibility JSON workspaces. Then the narrative must be reviewed/approved or explicitly overridden before updating the deck projection.
