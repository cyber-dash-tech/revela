...
在你的本地 workspace 中，Revela 会审阅本地资料、保存 source-linked research、生成明确的 `deck-plan.md`、产出 HTML deck，在 Codex Browser 中打开以便 annotation，并支持 PDF/PPTX/PNG 导出。
...
8. make-deck 打开生成后的 deck 后，在 Codex Browser 中做 annotation 和定向修改。
...
`revela-make-deck` 生成 HTML deck 且 Artifact QA 通过后，会在 Codex Browser 中打开 deck。使用 Codex Browser 原生 annotation 工具标注 layout、文案、层级、间距或视觉修改。
