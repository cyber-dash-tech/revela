...
- `decks/deck-plan.md`, once generated and approved, is the deck execution blueprint for slide order, chapter writing batches, visual intent, and evidence trace; it is not the source of canonical meaning.
- `DECKS.json.slides[]` must not be treated as the authoritative HTML slide-count contract. Artifact identity is self-consistent positive 1-based slide indexes, unique indexes, DOM order, and canvas validity; plan completeness belongs to `decks/deck-plan.md`.
...
- ~~Deck-plan artifact contract: `compileDeckPlan` prepares the canonical claim/evidence planning packet and deck-plan authoring requirements; the LLM writes `decks/deck-plan.md` as the human-readable execution blueprint with source authority, chapter map, slide plan, chapter writing batches, evidence trace, boundaries, HTML identity contract, and an Approval block. `readDeckPlan` reads the current artifact without regenerating it. `confirmDeckPlan` validates signed `status: approved`, `approvedBy`, `approvedAt`, current `narrativeHash`, and computed or matching `planHash` before recording deck-plan confirmation.~~
...
- ~~Deck-render skill contract: core `skill/SKILL.md` always carries deck writing rules in deck-render mode: generate 5+ slide decks chapter by chapter, never draft a full deck in one broad write/patch, preserve confirmed plan/narrative fidelity, keep claim-led chapters intact, avoid filler/divider padding, preserve evidence trace, keep fixed 1920x1080 canvas fit, avoid `scrollbar/page_scroll`, and fix Artifact QA hard errors before continuing.~~
...
- Do not require generated deck HTML to match cached `DECKS.json.slides[]` length during chapter-by-chapter authoring; partial artifacts are allowed when written slide identities are valid and the confirmed `decks/deck-plan.md` remains the execution guide.
