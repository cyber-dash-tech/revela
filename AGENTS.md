...
revela-narrative/
  ...
  risks/
  research-gaps/
  relations.md        # planned 0.17 closeout: centralized relation registry
```

Markdown authoring rules:

- `revela-narrative/**/*.md` is the LLM-editable knowledge workspace.
- The LLM may maintain loose Markdown nodes for claims, evidence, objections, risks, and research gaps. New graph edges should move through the relation registry direction below.
...
- Legacy inline relation syntax uses typed relation lines plus plain node-id wikilinks, for example `- supports: [[claim-recommendation]]`; do not add new inline `## Relations` sections when `relations.md` is available.
...
Planned 0.17 relation-registry direction:

- Move new graph relations out of node Markdown into `revela-narrative/relations.md` as a centralized edge registry.
- Treat inline `## Relations` as legacy-compatible input during migration, not the preferred new authoring path.
- Keep node files focused on node content; evidence files preserve source/quote/scope/caveat/strength, while support/challenge/constrain/answer edges live in the registry.
- Use relation sync QA to report dangling edges, unbound evidence/objections/risks/gaps, isolated central claims, duplicate edge ids, invalid relation types, and legacy inline relations at the right workflow checkpoints.
...
0.17 closeout goal: establish a reliable Markdown authoring control loop. Prompts constrain how the LLM writes Markdown before authoring; inventory tools show existing nodes before edits; hooks/tools run Markdown QA after authoring and return repair task cards; relation registry and sync QA keep graph edges aligned with nodes without forcing the LLM to write relations inside node files; `compileNarrativeVault` remains the deterministic graph compiler.

Do not move on to 0.17.1 until Phases 1-6 are closed. Phase 7 is the release verification gate.
...
### Phase 6: Relation Registry + Layered Relation Sync QA

Goal: stop asking the LLM to maintain graph edges inside claim/evidence/risk/objection/gap node files. Node Markdown remains the loose knowledge workspace; `relations.md` becomes the official edge registry; tools report relation/node drift at smart checkpoints instead of interrupting every small edit.

Design decisions:

- Add `revela-narrative/relations.md` as the preferred source for new graph edges.
- Keep legacy inline `## Relations` compile-compatible during migration, but prompts and helpers should stop generating new inline relation sections.
- Store relation edges separately from evidence/source content. Evidence nodes preserve source trace and support boundaries; registry edges say which claim they support.
- Do not rely on the LLM to remember relation sync. Use inventory, Markdown QA, and readiness gates to report drift.
- Relation sync cannot infer every missing semantic edge. It should reliably detect mechanical drift and coverage gaps, then ask the LLM/user to add or confirm the right relation.

Registry shape:

- Minimal edge fields: `id`, `from`, `to`, `type`, and optional `rationale`.
- Valid relation types stay aligned with canonical narrative relations: `leads_to`, `supports`, `depends_on`, `contrasts_with`, `constrains`, `answers`.
- Edge ids should be stable plain ids such as `rel-market-tension-to-new-frame` or `rel-proposal-intent-supports-belief-change`.
- `from` and `to` reference frontmatter node ids directly, not file paths and not typed wikilinks.

Layered sync strategy:

- Write hook scope: run touched-file structural QA only. Check relation file parseability, required edge fields, duplicate edge ids, invalid relation types, touched node id/type validity, and newly added inline `## Relations`. Do not run full orphan/isolated graph checks on every edit.
- Authoring checkpoint scope: `/revela init` final report, `/revela research` after a binding/narrowing batch, manual `revela-decks markdownQa`, and explicit story/readiness checks should run affected-subgraph or full relation sync. Report repair cards for dangling edges, unbound evidence, unbound objections, unbound risks, unbound research gaps, isolated central claims, and legacy inline relations.
- Readiness/render scope: `/revela story`, `/revela make --deck`, `/revela make --brief`, `approveNarrative`, and review/inspection flows should run full strict relation sync. Broken registry edges, unbound evidence support, and unreadable claim-flow should block rendering or approval instead of being hidden by prompts.
- `narrativeInventory` should expose `relationCoverage`: dangling edges, unbound evidence/objections/risks/gaps, isolated claims, orphan nodes, and relation candidates when safely derivable. Candidates are advisory and never official graph state until written to `relations.md` or a relation helper.
...
- ~~Compiler reads `relations.md` edges and produces the same canonical claim/evidence/risk/objection/gap relationships as legacy inline relations.~~
