...
- ~~Deck-plan artifact contract: `compileDeckPlan` writes `decks/deck-plan.md` as the human-readable execution blueprint with metadata, chapter map, slide plan, chapter writing batches, quality checks, and an Approval block. `confirmDeckPlan` validates signed `status: approved`, `approvedBy`, `approvedAt`, `planHash`, and `narrativeHash` before recording deck-plan confirmation.~~
