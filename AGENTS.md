...
- ~~Deck-plan projection contract: replace the single `decks/deck-plan.md` approval artifact with a root `deck-plan/` render-projection workspace. `compileDeckPlan` prepares the canonical claim/evidence planning packet and deck-plan authoring requirements; the LLM writes `deck-plan/index.md` plus `deck-plan/slides/*.md` as editable execution-plan Markdown. The plan files are render-layer projection nodes, not canonical narrative meaning. Do not require an Approval block or `confirmDeckPlan` gate; users decide when to proceed.~~
- ~~Deck-plan graph contract: `deck-plan/index.md` is the deck-level plan index with goal, audience, deck parameters, chapter map, writing batches, evidence/boundary policy, HTML identity contract, current `narrativeHash`, and slide-file inventory. Each `deck-plan/slides/*.md` file is one slide-level plan with frontmatter such as `type: deck-plan-slide`, stable `id`, positive 1-based `slideIndex`, title, chapter, layout, components, and structural flag. Slide files link to narrative nodes through `## Narrative Links` using plain wikilinks like `[[claim-id]]`, `[[evidence-id]]`, `[[risk-id]]`, `[[objection-id]]`, or `[[gap-id]]`; do not use typed wikilinks or canonical `## Relations` for deck-plan links.~~
